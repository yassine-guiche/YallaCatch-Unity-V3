...
using ApiOfflineAction = YallaCatch.API.OfflineAction;
...
namespace YallaCatch.Managers
{
    ...
    public class OfflineManager : MonoBehaviour
    {
        ...
        // Runtime connectivity/cache state (Header attributes are only valid on fields).
        public bool IsOnline { get; private set; } = true;
        public List<ApiOfflineAction> PendingActions { get; private set; }
        ...
        private void Awake()
        {
            if (Instance == null)
            {
                ...
                PendingActions = new List<ApiOfflineAction>();
                LoadPendingActions();
            }
            else
            {
        ...
        private void Start()
        {
            ...
            {
                Debug.Log("[OfflineManager] Skipping capability load until player is authenticated.");
            }
            else
            {
                LoadCapabilities();
        ...
        public void QueueAction(string actionType, object data)
        {
            ...
            var action = new ApiOfflineAction
            {
        ...
        private void LoadPendingActions()
        {
            ...
            try
            {
                PendingActions = Newtonsoft.Json.JsonConvert.DeserializeObject<List<ApiOfflineAction>>(json) ?? new List<ApiOfflineAction>();
            }
            catch
            {
                PendingActions = new List<ApiOfflineAction>();
