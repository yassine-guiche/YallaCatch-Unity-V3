...
namespace YallaCatch
{
    ...
    public class ChallengesManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                Debug.Log("[ChallengesManager] Skipping auto-initialize until player is authenticated.");
                return;
            }
        ...
        public IEnumerator LoadDailyChallenges(System.Action<List<DailyChallenge>> callback = null)
        {
            ...
            string endpoint = "/game/challenges";
        ...
        public IEnumerator CompleteChallenge(string challengeId, System.Action<ChallengeCompletionResult> callback = null)
        {
            ...
            string endpoint = $"/game/challenges/{challengeId}/complete";
            ...
            yield return APIManager.Instance.Post(endpoint, data, (response) =>
            {
                if (response.success)
                {
                    ...
                    if (challenge != null)
                    {
                        ...
                        if (result.reward > 0)
                        {
                            GameManager.Instance.AddPoints(result.reward);
                        }
        ...
        public void UpdateChallengeProgress(string challengeType, int value)
        {
            foreach (var challenge in dailyChallenges)
            {
                if (challenge.type == challengeType && !challenge.completed)
                {
                    ...
                    challenge.progress = challengeProgress[challenge.id].currentValue;
                    
                    // Calculer le pourcentage
                    float progress = (float)challenge.progress / challenge.target;
                    ...
                    Debug.Log($"[ChallengesManager] Progress updated: {challenge.title} - {challenge.progress}/{challenge.target}");
                }
            }
        }
    ...
    public class DailyChallenge
    {
        ...
        // Compatibility alias used by existing UI/gameplay code paths.
    ...
    public class ChallengeProgress
    {
        ...
        // Compatibility alias used by manager/UI code paths that expect targetValue.
