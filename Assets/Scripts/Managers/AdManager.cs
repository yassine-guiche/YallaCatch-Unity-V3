...
namespace YallaCatch.Managers
{
    ...
    public class AdManager : MonoBehaviour
    {
        ...
        // Inspector headers only apply to fields; Config/Stats are runtime properties.
        ...
        private void Start()
        {
            ...
            {
                Debug.Log("[AdManager] Skipping ad config load until player is authenticated.");
                return;
            }
        ...
        {
            // Backend has no separate "view-start" endpoint; /admob/reward only accepts completed=true.
            if (!completed)
            {
                callback?.Invoke(true);
                return;
        ...
        public void ClaimAdReward(string adType, string adUnitId, string viewId, System.Action<bool, int> callback)
        {
            ...
            AdMobAPI.Instance.ClaimReward(request, response =>
            {
                if (response.success && response.data != null)
                {
                    int points = response.data.GetAwardedPoints();
        ...
        {
            int dailyLimit = Config?.dailyRewardLimit ?? 5;
            return Stats != null ? Stats.GetRemainingDailyRewards(dailyLimit) : dailyLimit;
        }
        ...
        private void SimulateAdWatch(string adUnitId, System.Action<bool, int> callback)
        {
            ...
            // Backend uses a single /reward endpoint for completed ads.
