...
namespace YallaCatch.Managers
{
    ...
    public class RewardsManager : MonoBehaviour
    {
        ...
        {
            return CachedRewards != null ? new List<Reward>(CachedRewards) : new List<Reward>();
        }
        ...
        public bool CanAffordReward(Reward reward)
        {
            ...
            return GameManager.Instance.PlayerPoints >= reward.pointsCost;
        }
        ...
        public void RedeemReward(string rewardId, System.Action<bool, Redemption, string> callback)
        {
            ...
            string idempotencyKey = $"REDEEM_{rewardId}_{GameManager.Instance.PlayerId}_{DateTime.UtcNow.Ticks}";

            RewardsAPI.Instance.RedeemReward(rewardId, idempotencyKey, null, response =>
            {
                if (response.success && response.data != null)
                {
        ...
        public void RedeemPromoCode(string code, System.Action<bool, string, PromoCodeResponse> callback)
        {
            RewardsAPI.Instance.RedeemPromoCode(code, response =>
            {
                if (response.success && response.data != null)
                {
                    ...
                    {
                        // Potential future logic: Add to local "MyRedemptions" or inventory
                        Debug.Log($"[RewardsManager] Promo code granted reward: {response.data.reward.name}");
                    }

                    callback?.Invoke(true, response.data.message ?? "Promo code redeemed!", response.data);
                }
                else
                {
                    callback?.Invoke(false, response.message ?? "Invalid promo code", null);
                }
            });
        }
