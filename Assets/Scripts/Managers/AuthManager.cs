...
namespace YallaCatch.Managers
{
    ...
    public class AuthManager : MonoBehaviour
    {
        ...
        private void OnLoginSuccess(User user)
        {
            if (user == null)
            {
                ShowError("Authentication succeeded but user profile was empty");
                return;
            }

            if (string.IsNullOrWhiteSpace(user.displayName))
            {
                user.displayName = user.DisplayNameSafe;
            }

            if (APIClient.Instance == null || !APIClient.Instance.IsAuthenticated)
            {
                ShowError("Authentication token was not saved correctly. Please try again.");
                UI.UIManager.Instance?.ShowLoginPanel();
                return;
            }
            ...
            // Realtime connection is deferred until auth is confirmed.
            StartCoroutine(ConnectRealtimeAfterAuth());
        }

        #endregion

        private IEnumerator ConnectRealtimeAfterAuth()
        {
            // Give APIClient token state one frame to settle in case the auth callback ordering changes.
            yield return null;

            float timeout = 2f;
            float elapsed = 0f;
            while (APIClient.Instance != null && string.IsNullOrEmpty(APIClient.Instance.GetAccessToken()) && elapsed < timeout)
            {
