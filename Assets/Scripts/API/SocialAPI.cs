...
namespace YallaCatch.API
{
    ...
    public class SocialAPI : MonoBehaviour
    {
        ...
        public void GetNearbyPlayers(Location location, float radiusKm = 5f, Action<ApiResponse<NearbyPlayersResponse>> callback = null)
        {
            ...
            string radius = radiusKm.ToString(CultureInfo.InvariantCulture);
            string query = $"?latitude={Uri.EscapeDataString(latitude)}&longitude={Uri.EscapeDataString(longitude)}&radius={Uri.EscapeDataString(radius)}";
