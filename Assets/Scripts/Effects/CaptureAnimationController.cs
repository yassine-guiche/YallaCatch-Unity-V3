...
namespace YallaCatch
{
    ...
    public class CaptureAnimationController : MonoBehaviour
    {
        ...
        {
            if (prizeBoxPrefab == null || boxSpawnPoint == null) yield break;
            Camera mainCamera = Camera.main;
            if (mainCamera == null)
            {
                Debug.LogWarning("[CaptureAnimation] Main camera not found, skipping prize box flight animation.");
                yield break;
            }
            ...
            if (boxIcon != null)
            {
                // Tint icon by rarity color
                boxIcon.color = GetRarityColor(prize.rarity);
            }
            
            // Position de départ et d'arrivée
            Vector3 startPos = boxSpawnPoint.position;
            Vector3 endPos = mainCamera.transform.position + mainCamera.transform.forward * 2f;
        ...
        private void PlayParticleEffects()
        {
            ...
            if (mainCamera == null)
            {
                Debug.LogWarning("[CaptureAnimation] Main camera not found, particle placement skipped.");
                return;
            }

            Vector3 playerPos = mainCamera.transform.position + mainCamera.transform.forward * 2f;
        ...
        private void ShowRewardPopup(Prize prize, int pointsEarned)
        {
            ...
            if (rewardIcon != null)
            {
                // Tint reward icon by rarity color
                rewardIcon.color = GetRarityColor(prize.rarity);
            }
        ...
        /// <summary>
        /// Returns a color based on prize rarity
        /// </summary>
        private Color GetRarityColor(string rarity)
        {
            ...
            {
                case "common":
                    return new Color(0.7f, 0.7f, 0.7f); // Silver grey
                case "uncommon":
